...
/**
  ...
  * @param  NumByteToRead: number of bytes to read from the FLASH.--ok
  * @retval None
  */
...
/**
  ...
  * @param  None
  * @retval Byte Read from the SPI Flash.--ok
  */
...
void SPI_transfer_block(uint8_t *pBuffer_tx, uint16_t NumByteToTx,uint8_t *pBuffer_rx, uint16_t NumByteToRx)
{
	// while (NumByteToRead--) /*!< while there is data to be read */
	// {
	// 	/*!< Read a byte from the FLASH */
	...
	while (NumByteToTx--)
	{
		sFLASH_SendByte(*pBuffer_tx++);
	}

	while (NumByteToRx--)
	{
		*pBuffer_rx = sFLASH_SendByte(sFLASH_DUMMY_BYTE);
		/*!< Point to the next location where the byte read will be saved */
		pBuffer_rx++;
	}
}
