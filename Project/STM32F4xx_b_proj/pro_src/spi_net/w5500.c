...
for(i=0;i<6;i++)RJ45_config.mac[i]=default_mac[i];
...
for(i=0;i<num;i++)
 {
  for(j=0;j<5;j++)
	 {
      cdata[j]=*config++;
      if(cdata[j]=='.'||cdata[j]=='}'||cdata[j]==')')break;
      if(cdata[j]>'9' || cdata[j]<'0')return ;
    //  config++;
	 }
...
for(i=0;i<num;i++)
 {
  for(j=0;j<5;j++)
	 {
      cdata[j]=*config++;
      if(cdata[j]=='.'||cdata[j]=='}'||cdata[j]==')'||cdata[j]==',')break;
       //  config++;
	 }
...
//	set_netpara_default();
	i=getVersion();
//	debug("netVer",i);
    for(i=0;i<6;i++)tmp_data[i]=RJ45_config.mac[i];
 	setSHAR(tmp_data);/*����Mac��ַ*/

	for(i=0;i<4;i++)tmp_data[i]=RJ45_config.local_ip[i];
    setSIPR(tmp_data);/*����Ip��ַ*/

	for(i=0;i<4;i++)tmp_data[i]=RJ45_config.mask[i];
	setSUBR(tmp_data);/*������������*/

    for(i=0;i<4;i++)tmp_data[i]=RJ45_config.gateway[i];
	setGAR(tmp_data);/*����Ĭ������*/
...
//   W5500_CS_LOW;                              // CS=0, SPI start
   SPI_set_slave_select(net_core_spi, w5500_cs);
...
//  wait_ready();
  ...
  //  wait_ready();
   SPI_clear_slave_select(net_core_spi, w5500_cs);
