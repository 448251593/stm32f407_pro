...
/***************----- 端口数据缓冲区 -----***************/
unsigned char Rx_Buffer[1460];	//端口接收数据缓冲区 
unsigned char Tx_Buffer[1460];	//端口发送数据缓冲区 
...
{
	sNET_SendByte(dat/256);//写数据高位
	sNET_SendByte(dat);	//写数据低位
}
...
{
	SPI_set_slave_select();//置W5500的SCS为低电平
	...
	SPI_clear_slave_select(); //置W5500的SCS为高电平
}
...
unsigned short Read_W5500_SOCK_2Byte(SOCKET s, unsigned short reg)
{
	...
	// i=SPI_I2S_ReceiveData(SPI1);
	// SPI1_Send_Byte(0x00);//发送一个哑数据
	i=sNET_ReadByte();//读取高位数据
	// SPI1_Send_Byte(0x00);//发送一个哑数据
	i*=256;
	i+=sNET_ReadByte();//读取低位数据
...
void W5500_Init(void)
{
	...
	Write_W5500_SOCK_1Byte(0,Sn_RXBUF_SIZE, 0x08);//Socket Rx memory size=2k
	Write_W5500_SOCK_1Byte(0,Sn_TXBUF_SIZE, 0x08);//Socket Tx mempry size=2k
	...
	Write_W5500_SOCK_1Byte(2,Sn_RXBUF_SIZE, 0x02);//Socket Rx memory size=2k
	Write_W5500_SOCK_1Byte(2,Sn_TXBUF_SIZE, 0x02);//Socket Tx mempry size=2k
	Write_W5500_SOCK_1Byte(3,Sn_RXBUF_SIZE, 0x02);//Socket Rx memory size=2k
	Write_W5500_SOCK_1Byte(3,Sn_TXBUF_SIZE, 0x02);//Socket Tx mempry size=2k
	Write_W5500_SOCK_1Byte(4,Sn_RXBUF_SIZE, 0x02);//Socket Rx memory size=2k
	Write_W5500_SOCK_1Byte(4,Sn_TXBUF_SIZE, 0x02);//Socket Tx mempry size=2k
	Write_W5500_SOCK_1Byte(5,Sn_RXBUF_SIZE, 0x00);//Socket Rx memory size=2k
	Write_W5500_SOCK_1Byte(5,Sn_TXBUF_SIZE, 0x00);//Socket Tx mempry size=2k
	Write_W5500_SOCK_1Byte(6,Sn_RXBUF_SIZE, 0x00);//Socket Rx memory size=2k
	Write_W5500_SOCK_1Byte(6,Sn_TXBUF_SIZE, 0x00);//Socket Tx mempry size=2k
	Write_W5500_SOCK_1Byte(7,Sn_RXBUF_SIZE, 0x00);//Socket Rx memory size=2k
	Write_W5500_SOCK_1Byte(7,Sn_TXBUF_SIZE, 0x00);//Socket Tx mempry size=2k


	//设置重试时间，默认为2000(200ms) 
	//每一单位数值为100微秒,初始化时值设为2000(0x07D0),等于200毫秒
	Write_W5500_2Byte(RTR, 1000);
